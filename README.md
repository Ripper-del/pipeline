...
3. Запуск сервисов по отдельности:
   ...
   * Парсер TikTok:
    ...
    * Автоматизация и прогрев TikTok (через AdsPower):
      ...
      Для генерации контекстных комментариев с помощью ИИ укажите `OPENAI_API_KEY` в файле `.env`.
      ...
      **Особенности автоматизатора:**
      ...
      - **Поддержка 15 ГЕО/языков**: Встроенная база данных поддерживает US, UK, DE, FR, ES, IT, BR, NL, PL, TR, RO, JP, KR, VN, CA. Для каждого ГЕО прописаны целевые поисковые запросы по теме дейтинга/отношений на локальном языке.
      - **Очеловеченные комментарии**: Комментирование осуществляется строго через нейросеть (OpenAI GPT). Бот на базе описания видео генерирует короткий разговорный комментарий без смайликов на языке целевого ГЕО. Указание `OPENAI_API_KEY` в файле `.env` является обязательным для запуска автоматизатора.
