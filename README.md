...
3. Запуск сервисов по отдельности:
   ...
   * Парсер TikTok:
    ...
    * Автоматизация и прогрев TikTok (через AdsPower):
      ...
      1. Прогрев аккаунта (`warmup`): имитация реального пользователя. Бот ищет видео по дейтинг-тематике для выбранного ГЕО (доступно 15 ГЕО), задерживается на их просмотре (прогрев удержания), ставит лайки и оставляет человекоподобные комментарии на нужном языке, настраивая алгоритм рекомендаций TikTok под вертикаль Adult Dating.
      2. Режим Спая (`spy`): мониторинг ленты или поиска. Бот находит целевые креативы и ставит лайк/оставляет комментарий на каждое 5-е подходящее видео.
